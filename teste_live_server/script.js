...
function showTab(tabId) {
  ...
  if (tabId === 'transformacao-int') {
    setTimeout(() => updateTransformacaoViz(), 50);
  }
}
...
function mudancaBase() {
  ...
  if (det === 0) {
    ...
    <div class="p-4 rounded-xl bg-red-500/20 border border-red-500/30">
      ...
      Os vetores da base são LD (det = 0)
      </p>
      <p class="text-gray-400 text-center mt-2">
      Não formam uma base válida
      </p>
    </div>`;
  } else {
...
function transformacaoLinear(){
  const t1x = parseFloat(document.getElementById('t1x').value) || 1;
  const t1y = parseFloat(document.getElementById('t1y').value) || 0;
  const t2x = parseFloat(document.getElementById('t2x').value) || 1;
  const t2y = parseFloat(document.getElementById('t2y').value) || 0;
  const tvx = parseFloat(document.getElementById('tvx').value) || 1;
  const tvy = parseFloat(document.getElementById('tvy').value) || 0;
  const twx = (t1x * tvx) + (t1y * tvy);
  const twy = (t2x * tvx) + (t2y * tvy);
}
...
function startDragging(event, tabId) {
  ...
  const vec = document.getElementById(`${tabId}-vec`);
  const label_v = document.getElementById(`${tabId}-label-v`);
  const marker_v = document.getElementById(`arrow-${tabId}-v`);
  
  // Check if clicking on the vector line or its label for vector dragging
  if (event.target === vec || event.target === label_v || event.target === marker_v) {
    dragMode = 'vector';
    isDragging = true;
    svg.style.cursor = 'grabbing';
  } else if (event.target.id === `${tabId}-grid-background` || event.target === svg) {
...
function dragVector(event, vX, vY) {
