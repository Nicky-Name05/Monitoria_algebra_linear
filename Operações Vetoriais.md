Além das operações de soma e de multiplicação, - que precisam existir em um [Espaço Vetorial](<Vetor.md>)[^1]  - existem algumas outras operações que podem ser feitas com vetores e a maioria delas são parecidas com as [operações matriciais](<Operações de Matrizes.md>)[^2] que foram vistas antes ou vão usar as operações matriciais para resolvê-las. As operações vetoriais são feitas entre elementos pertencentes a um espaço vetorial.
...
$$ \overrightarrow v \cdot \overrightarrow f = a;$$

Em que $a$ é um escalar e ela pode ser resolvida das seguintes formas:
$$Sendo\ \overrightarrow v = v_x\hat i+v_y\hat j+v_z \hat k\ e\  \overrightarrow u=u_x\hat i+u_y\hat j+u_z\hat k,\ $$
...
$$  v\cdot f \xlongequal n a,\ em\ que\ 'a'\ vem\ de\
\begin{bmatrix}
v_x&v_y&v_z
\end{bmatrix}\cdot
\begin{bmatrix}
...
$$ v\times u= {|v|\cdot|u|\cdot sen(\theta)};$$
...
>Como quase todos os polígonos podem ser formados por uma combinação de triângulos - e por isso suas áreas podem ser escritas como a soma das áreas desse triângulos ($\sum_{i=0}^n S_{\triangle_i}$) - e a área de um triângulo pode ser escrita como $\frac {a \cdot b\cdot sen (\theta)} 2$, a área de um triângulo também pode ser escrita como:
>
>$$ S_{\triangle}= 
\frac 1 2 \cdot|
\begin{vmatrix}
...
$$Sejam\ \overrightarrow v, \overrightarrow u, \overrightarrow w\ três\ vetores,$$

$$ \overrightarrow v \cdot (\overrightarrow u\times \overrightarrow w) $$

>[!note]
>**Exemplo**\
>Produto misto de $\overrightarrow v=(1,3,2)$ escalar $\overrightarrow u=(3,0,2)$ vetorial $\overrightarrow w=(4,1,0)$:
>
>$$ {v_f}= 
\overrightarrow v \cdot (\overrightarrow u \times \overrightarrow w)=
\begin{bmatrix}
