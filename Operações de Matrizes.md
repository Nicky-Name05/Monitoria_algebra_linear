...
>[!tldr] Propriedades da Diferença
> 1. $A-B=-B+A$
> 2. $(A+B)-C=A+(B-C) = (A-C)+B$
> 3. Sendo $0$ a matriz nula nxm, $A-0=A$
...
>[!tldr] Propriedades do Produto Escalar
...
>[!tldr] Propriedades do Produto Matricial
> 1. $AB\ne BA$
> 2. $I_nA=AI_n=A$
> 3. $A(B + C) = AB + AC$
> 4. $(B + C)A = BA + CA$
...
>[!tldr] Propriedades da Transposição de Matrizes
>1. $(A^t)^t=A$
>2. $(A+B)^t=A^t+B^t$
...
>[!attention] Só existe determinante de matrizes quadradas. 

Existem diversas formas de se calcular o determinante, mas a forma mais usada é a [[Regra de Laplace]], e um dos principais motivos para se fazer esse cálculo é para achar a solução de um [[Sistema Linear]].

>[!tldr] Propriedades do Determinante
...
= 0$$
>
>6. Se é uma matriz quadrada $B$ é uma matriz obtida de $A$ através da multiplicação por um número real $k$ de uma linha ou de uma coluna, então o determinante de $B$ é $k$ vezes o determinante de $A$.Ex: $det(B) = k \cdot det(A)$.
>>[!important] Se $B = k \cdot A$, $det(B)=k^n det(A)$, em que n é a ordem de $A$ e $B$.
...
>$$\Rightarrow-9=-15+6$$
>
>>[!important] Apesar disso, em geral, $det(A+B) \ne det(A)+det(B)$.
...
>[!attention] Nem toda matriz tem inversa
...
Existe duas formas principais de se [calcular a inversa](<Cálculo da inversa>):
