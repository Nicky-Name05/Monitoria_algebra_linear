...
3. Existe um $\require{physics} \va 0$, tal que $\va 0 \oplus v = v$ é uma afirmação verdadeira (Elemento Neutro da Adição)
4. Existe um $-v$, tal que $v \oplus (-v) = \va 0$ é uma afirmação verdadeira e $\va 0$ é o Elemento Neutro da Adição
...
8. $\va 1 \otimes v=v$ (Elemento Neutro da Multiplicação)

>[!important] As operações de soma e multiplicação por escalar podem ser diferentes da padrão
...
>
>Quando a adição vetorial e a multiplicação por escalar são a padrão também costuma-se usar os símbolos $+$ e $\vdot$ (seus símbolos normais).

>[!attention] $\va 0$ e $\va 1$ podem ser diferentes dos escalares $0$ e $1$
...
>[!example] Exemplo 
>Suponha um conjunto $\mathbb V = (0,+\infty)$, com as operações $x\oplus y= x\vdot y$ e $\lambda \otimes x= x^\lambda$, é possível considerá-lo um Espaço Vetorial, pois:  
>
>$$Seja\; v \ e \ u \in \mathbb V \ e\ \lambda,\lambda_2 \in \mathbb R,$$ 
>
>$u \oplus v = u\vdot v = v \vdot u = v \oplus u$ (Segunda condição é atendida. Por indução a Primeira também é).
>
>$\va0\oplus v=v \Rightarrow \va 0\vdot v = v\Rightarrow \va 0 = 1$ (Terceira condição é atendida)
>
>$v\oplus (-v)= \va 0\Rightarrow v\vdot (-v) = 1\Rightarrow (-v) = \frac 1 v\Rightarrow -v \in V$ (Quarta condição é atendida)
>
> $\lambda\otimes(u\oplus v)=(u \vdot v)^{\lambda}=(u^{\lambda})\vdot (v^{\lambda})=(\lambda \otimes u )\oplus (\lambda \otimes v$ )(Quinta condição é atendida)
>
> $(\lambda + \lambda_2)\otimes v= v^{\lambda+\lambda_2}=v^{\lambda}\vdot v^{\lambda_2}=(v^{\lambda})\oplus(v^{\lambda_2})=(\lambda\otimes v)\oplus(\lambda\otimes v)$ (Sexta condição é atendida)
>
>$(\lambda \vdot \lambda_2)\otimes v= v^{\lambda\vdot\lambda_2}=(v^{\lambda_2})^{\lambda}=(\lambda_2\otimes v)^{\lambda}=\lambda\otimes(\lambda_2\otimes v)$ (Sétima condição é atendida)
>
> $\va 1 \otimes v=v\Rightarrow  v^{\va 1}=v\Rightarrow \va 1= 1\Rightarrow \va 1\in \mathbb R$ (Oitava condição é atendida).
## Subespaço Vetorial
...
2. Se $v\ \in\ \mathbb W$ e $\lambda \ \in \ \mathbb R$, $\lambda \vdot v\ \in \ \mathbb W$ (Diz-se que "W é fechado para produto por escalar", ou seja, o produto de qualquer vetor de $\mathbb W$ com com um escalar está contido em $\mathbb W$).

>[!info] Subespaços triviais
...
> [!important] Não é necessário conferir as condições de Espaço Vetorial em um Subespaço Vetorial
...
> $$
 ...
 >$$\lambda \vdot v= \lambda \vdot (0,\ v_1,\ v_2,\ v_3)= (0,\ \lambda v_1,\ \lambda v_2,\ \lambda v_3),\ que\ está\ em\ \mathbb W.$$
 >---
